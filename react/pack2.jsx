...
const { data: data2, loading } = useQuery(QUERY_SETTINGS, { ssr: false, onCompleted: async() => { 
  ...
  console.log("INFORMACION CARGADA DESDE GRAPHQL 2", data2)
  const result = JSON.parse(data2.appSettings.message);
  console.log("RESULT 2", result);
  const resultado = await axios.get(`/api/catalog_system/pub/products/search?fq=productClusterIds:${result.idCollection2}`);
  ...
  console.log("PRODUCTOS FILTRADOS DESPUES PUSH 2",arrayAux);
  setProductosFiltrados(arrayAux);
  console.log("PRODUCTOS FILTRADOS DESPUES SET 2",productosFiltrados);
}} })
...
*/
  var startDate = moment(settingsP.startdate2);
  //console.log("AÑO",stardDate);
  
  var endDate = moment(settingsP.enddate2);
...
function mostrarOferta(){
  ...
  if(hide === "none" && terminado===false && horaF > horaA){
    console.log("MOSTRAR IF 3 CONDICIONES 2")
    setHide("block")
  }
...
function mostrarPrimeraCargar(){
  horaA = moment().format();
if(horaA === horaI){
  console.log("ENTRO IF MOSTRAR PRIMERA CARGA 2");
  setHide("block");
}else if(horaA < horaI || horaA > horaF){
  console.log("ELSE IF MOSTRAR PRIMERA CARGA 2")
  if(horaA > horaF){
    setHide("none");
  }
}
}
  var collect2 = parseInt(settingsP.idCollection2);
    ...
    <div className={`${count.ofertaContainer}`} style={{display: hide}}>
      ...
      <div className={`${count.ofertaHeader}`}>
        ...
        <div id="countdown-body" className={`${count.countdownbody}`}>
          ...
          <Countdown2 inicio={startDate} termino={endDate} setHide={setHide} setTerminado={setTerminado} handles={CSS_HANDLES}/>
          </div>
